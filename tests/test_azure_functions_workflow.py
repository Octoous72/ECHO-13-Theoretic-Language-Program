"""
test_azure_functions_workflow.py - Azure Functions Workflow Configuration Tests
================================================================================
...
The PR introduced structural defects:
  ...
  - The `Run Azure Functions Action` step was embedded inside the run: shell
    script instead of being a top-level step
...
class TestAzureWorkflowFileExistence:
    ...
    def test_file_is_utf8_readable(self) -> None:
        try:
            ...
            assert False, f"azure-functions-app-python.yml is not readable as UTF-8: {exc}"
...
class TestAzureWorkflowYAMLValidity:
    ...
    def test_workflow_name_is_correct(self) -> None:
        data = _load_workflow_yaml()
        assert "Deploy Python project to Azure Function App" in data.get("name", ""), (
...
class TestAzureWorkflowEnvVariables:
    ...
    def test_azure_functionapp_name_env_defined(self) -> None:
        data = _load_workflow_yaml()
        env = data.get("env", {})
        assert "AZURE_FUNCTIONAPP_NAME" in env, (
    ...
    def test_azure_functionapp_package_path_env_defined(self) -> None:
        data = _load_workflow_yaml()
        env = data.get("env", {})
        assert "AZURE_FUNCTIONAPP_PACKAGE_PATH" in env, (
    ...
    def test_python_version_env_defined(self) -> None:
        ...
        assert "PYTHON_VERSION" in env, (
            "PYTHON_VERSION must be defined in env"
        )
...
class TestAzureWorkflowJobStructure:
    ...
    def test_build_and_deploy_job_exists(self) -> None:
        ...
        assert "build-and-deploy" in jobs, (
            "Expected 'build-and-deploy' job in workflow"
        )
    ...
    def test_build_and_deploy_has_expected_number_of_steps(self) -> None:
        """The job should have 4 distinct steps, not fewer (which would indicate
        the Azure Functions step got absorbed into the pip-install run block)."""
        data = _load_workflow_yaml()
        job = data["jobs"]["build-and-deploy"]
        steps = job.get("steps", [])
        assert len(steps) >= 4, (
            f"Expected at least 4 steps in build-and-deploy, found {len(steps)}. "
            "The 'Run Azure Functions Action' step may have been embedded inside "
...
class TestAzureFunctionsActionStep:
    """
    Tests that the 'Run Azure Functions Action' step is a proper top-level step,
    not embedded inside the pip-install run: shell script block.
    ...
    def _get_azure_functions_step(self) -> dict | None:
        for step in self._get_steps():
            if isinstance(step, dict):
                uses = step.get("uses", "")
                name = step.get("name", "")
                if "functions-action" in str(uses) or "Run Azure Functions Action" in str(name):
                    return step
        return None

    def test_run_azure_functions_action_step_name_present_in_file(self) -> None:
        text = _load_workflow_text()
        assert "Run Azure Functions Action" in text, (
    ...
    def test_azure_functions_action_is_proper_step_not_shell_text(self) -> None:
        """The Azure Functions Action must appear as a parsed step dict, not as
        shell script text embedded inside the pip-install run: block."""
        step = self._get_azure_functions_step()
        assert step is not None, (
            "No step with 'uses: Azure/functions-action' found in the parsed YAML steps. "
            "The step may have been embedded inside the pip-install run: shell block "
            "rather than being a top-level workflow step."
        )

    def test_azure_functions_step_uses_correct_action(self) -> None:
        step = self._get_azure_functions_step()
        assert step is not None, "Azure Functions Action step not found"
        assert "Azure/functions-action" in step.get("uses", ""), (
    ...
    def test_azure_functions_step_uses_v1(self) -> None:
        step = self._get_azure_functions_step()
        assert step is not None, "Azure Functions Action step not found"
        assert step.get("uses", "").endswith("@v1"), (
    ...
    def test_azure_functions_step_id_is_fa(self) -> None:
        step = self._get_azure_functions_step()
        assert step is not None, "Azure Functions Action step not found"
        assert step.get("id") == "fa", (
            f"Expected step id 'fa', got '{step.get('id')}'"
        )
...
class TestAzureFunctionsActionWithBlock:
    """Tests that the 'with:' block for the Azure Functions step contains
    ...
    def test_publish_profile_parameter_present(self) -> None:
        with_block = self._get_with_block()
        assert "publish-profile" in with_block, (
    ...
    def test_scm_do_build_during_deployment_parameter_present(self) -> None:
        with_block = self._get_with_block()
        assert "scm-do-build-during-deployment" in with_block, (
    ...
    def test_enable_oryx_build_parameter_present(self) -> None:
        with_block = self._get_with_block()
        assert "enable-oryx-build" in with_block, (
    ...
    def test_enable_oryx_build_is_true(self) -> None:
        ...
        assert val is True, (
            f"Expected enable-oryx-build to be true, got {val!r}"
        )
...
class TestAzureWorkflowStructuralDefects:
    ...
    def test_client_id_not_in_pip_install_run_block(self) -> None:
        """'client-id' must NOT appear as bare text inside the pip-install shell script.
        The PR incorrectly inserted it after 'popd' in the run: block."""
        data = _load_workflow_yaml()
        steps = data["jobs"]["build-and-deploy"].get("steps", [])
        for step in steps:
            ...
            if "pip install" in str(run_script) or "popd" in str(run_script):
                assert "client-id" not in str(run_script), (
                    "Found 'client-id' as bare text inside the pip-install run: shell block. "
                    "This is a structural defect from the PR."
                )

    def test_no_invalid_dash_with_key_in_file(self) -> None:
        """'-with:' is not valid YAML for a step parameter; the correct key is 'with:'."""
        text = _load_workflow_text()
        assert "-with:" not in text, (
            "Found '-with:' in workflow file. This is invalid YAML syntax introduced "
            "by the PR (should be 'with:')."
        )

    def test_azure_functions_action_not_embedded_in_shell_run_block(self) -> None:
        """The 'Azure/functions-action' reference must not appear inside any run: shell block."""
        data = _load_workflow_yaml()
        steps = data["jobs"]["build-and-deploy"].get("steps", [])
        for step in steps:
            if not isinstance(step, dict):
            ...
            assert "functions-action" not in str(run_script), (
                "Found 'Azure/functions-action' embedded inside a run: shell script block. "
                "The step definition was incorrectly placed inside the pip-install run block."
            )

    def test_run_azure_functions_action_name_not_inside_shell_script(self) -> None:
        """'Run Azure Functions Action' step name must not appear as shell script text."""
        data = _load_workflow_yaml()
        steps = data["jobs"]["build-and-deploy"].get("steps", [])
        for step in steps:
            if not isinstance(step, dict):
            ...
            assert "Run Azure Functions Action" not in str(run_script), (
                "Found 'Run Azure Functions Action' inside a run: shell script. "
                "The step was incorrectly embedded inside the pip-install run: block."
            )
    ...
    def test_pip_install_step_ends_at_popd(self) -> None:
        ...
        assert "uses:" not in pip_step_run, (
            "Found 'uses:' inside the pip-install run: block — step definition leaked into shell script"
        )
        assert "- name:" not in pip_step_run, (
            "Found '- name:' inside the pip-install run: block — step definition leaked into shell script"
        )

    def test_step_id_fa_present_on_azure_functions_step(self) -> None:
        """Regression: the 'id: fa' field was removed by the PR defect and must be restored."""
        data = _load_workflow_yaml()
        steps = data["jobs"]["build-and-deploy"].get("steps", [])
        fa_step = None
        for step in steps:
...
class TestAzureWorkflowPreExistingJobIntegrity:
    ...
    def test_resolve_dependencies_step_present(self) -> None:
        text = _load_workflow_text()
        assert "Resolve Project Dependencies Using Pip" in text, (
    ...
    def test_python_packages_target_path_in_pip_install(self) -> None:
        text = _load_workflow_text()
        assert ".python_packages/lib/site-packages" in text, (
    ...
    def test_workflow_environment_is_dev(self) -> None:
        data = _load_workflow_yaml()
        job = data["jobs"]["build-and-deploy"]
        assert job.get("environment") == "dev", (
...
class TestAzureWorkflowBoundaryAndRegression:
    ...
    def test_azure_functions_action_not_pinned_to_sha(self) -> None:
        ...
        text = _load_workflow_text()
        assert "Azure/functions-action@v1" in text, (
            "Azure/functions-action should be pinned to @v1"
        )

    def test_no_duplicate_client_id_key_outside_run_block(self) -> None:
        """'client-id' should not appear as a bare YAML key in the workflow steps.
        The PR introduced it as stray text after 'popd'."""
        lines = _load_workflow_lines()
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
            # A bare 'client-id' line (not a YAML key like 'client-id:' with a value,
            # and not inside a comment) is invalid in a step context
            if stripped == "client-id":
                assert False, (
                    f"Line {i}: Found bare 'client-id' text in workflow file. "
                    "This was incorrectly inserted after 'popd' in the pip-install run: block."
                )
    ...
    def test_workflow_has_no_tab_characters(self) -> None:
        ...
        for i, line in enumerate(lines, 1):
            if line.startswith("\t"):
                assert False, (
                    f"Line {i} starts with a tab character, which is invalid in YAML"
                )
    ...
    def test_azure_functions_step_uses_key_appears_after_name_key(self) -> None:
        ...
        for i, line in enumerate(lines):
            ...
            if "Azure/functions-action@v1" in line and uses_line_idx is None:
                uses_line_idx = i
        assert name_line_idx is not None, "'Run Azure Functions Action' not found in file"
        assert uses_line_idx is not None, "'Azure/functions-action@v1' not found in file"
        assert uses_line_idx > name_line_idx, (
            "'uses: Azure/functions-action@v1' should appear after the step name declaration"
        )
        assert uses_line_idx - name_line_idx <= 5, (
            f"'uses:' appeared {uses_line_idx - name_line_idx} lines after 'name:', expected <= 5"
        )
